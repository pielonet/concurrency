...
class Queue {
     ...
     */
    private function generateLinearArrivalTimes($clients_count_max, $office_open_duration) {
    ...
    public function singleQueue(array $params) {
        ...
        if ($arrival_probability_check == 'normal') {
            ...
            $arrival_times = $this->generateLinearArrivalTimes($clients_count_max, $office_open_duration);
        }
        ...
        while (true) {
            ...
            if ($client_enters_queue) {
                ...
                $this->logger(count($queue) . " client(s) in queue\n");
            }
        ...
        $this->logger("Average wait duration: {$average_wait_duration}s\n");
    ...
    public function multipleQueue(array $params) {
        ...
        while (true) {
            ...
            if ($client_enters_queue) {
                ...
                $this->logger("ClientId $clients_entered_count enters QueueId $queue_id. Queue length is " . count($queues[$queue_id]) . ".\n");
            }
            foreach ($desks as $desk_id => &$client_id) {
                if ($client_id) {
                    $client = $clients[$client_id];
                ...
                if (is_null($client_id)) {
                    ...
                    if (!is_null($new_client_id)) {
                        ...
                        $this->logger("ClientId $client_id enters deskId $desk_id for {$desk_duration}s. ");
                        $this->logger("Client waited {$client['queue_wait_duration']}s in queue. ");
                        $this->logger("Queue length is " . count($queues[$desk_id]) . ".\n");
                        unset($client);
                    }
                }
            }
